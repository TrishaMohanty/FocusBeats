...
export function SessionSummaryModal({ isOpen, onClose, summaryData }: SessionSummaryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden border border-outline-variant/10 animate-in slide-in-from-bottom flex flex-col items-center p-12 text-center">
        ...
        <div className="grid grid-cols-2 gap-4 w-full mb-8">
            <div className="bg-surface-container-low rounded-[24px] p-6 border border-outline-variant/5">
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Time Studied</p>
        ...
        <div className="bg-surface-container-low rounded-2xl px-6 py-4 w-full mb-8 flex flex-col items-center justify-center border border-outline-variant/10">
             <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Task Addressed</p>
             <p className="font-bold text-sm line-clamp-1">{summaryData.task_name || 'Deep Work Flow'}</p>
        </div>

        <button
            onClick={onClose}
            className="w-full py-5 bg-gradient-to-br from-[#191C1D] to-[#2E3132] text-white rounded-[24px] font-black tracking-tight text-lg shadow-xl shadow-black/20 hover:scale-[1.02] active:scale-95 transition-transform"
        >
            Return to Dashboard
        </button>
